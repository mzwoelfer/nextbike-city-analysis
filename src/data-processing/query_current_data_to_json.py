...
def preprocess(df):
    ...
    df["next_lat"] = np.where(
        df.bikeId == df.bikeId.shift(-1), df.latitude.shift(-1), None
    )
    df["next_lon"] = np.where(
        df.bikeId == df.bikeId.shift(-1), df.longitude.shift(-1), None
    )

    # if the bike has moved its a trip, otherwise the bike was accessible at its location
    df["mode"] = np.where(
        (df.latitude != df.next_lat) | (df.longitude != df.next_lon),
        MODE_TRIP,
        MODE_ACCESS,
    )

    # if bike Id is not the same as next, then this is the final time stamp and the next_timestamp is set to None
    df["end_timestamp"] = df.end_timestamp.where(
        df.bikeId == df.bikeId.shift(-1), None, axis=0
    )
