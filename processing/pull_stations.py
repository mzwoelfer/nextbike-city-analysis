...
def get_station_data_from_database():
...
def save_files_by_day(date, group, folder):
    ...
    json_file = os.path.join(folder, f"stations_{date}.json")

    with open(json_file, "w") as f:
        json.dump(stations_json, f, indent=4)

    csv_file = os.path.join(folder, f"stations_{date}.csv")
    group.to_csv(csv_file, index=False)


def main():
    parser = argparse.ArgumentParser(
        description="Pull station data from database"
    )
    parser.add_argument(
        "input_file",
        nargs="?",
    ...
    if args.input_file:
        ...
        stations = get_station_data_from_database()

    print(stations)

    grouped = stations.groupby("date")
    for date, group in grouped:
        save_files_by_day(date, stations, args.export_folder)
